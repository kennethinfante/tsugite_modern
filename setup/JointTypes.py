...
def produce_suggestions(joint_type: JointType, hfs: list) -> list:
    ...
    for i in range(len(hfs)):
        for j in range(joint_type.dim):
            for k in range(joint_type.dim):
...
def layer_mat_from_cube(joint_type: JointType, lay_num: int, n: int) -> ArrayLike:
    mat = np.ndarray(shape=(joint_type.dim, joint_type.dim), dtype=int)
    fdir = joint_type.mesh.fab_directions[n]
    for i in range(joint_type.dim):
        for j in range(joint_type.dim):
            ind = [i, j]
            zval = (joint_type.dim - 1) * (1 - fdir) + (2 * fdir - 1) * lay_num
            ind.insert(joint_type.sax, zval)
            mat[i][j] = int(joint_type.mesh.voxel_matrix[tuple(ind)])
    return mat


def pad_layer_mat_with_fixed_sides(mat, joint_type: JointType, n):
    pad_loc = [[0, 0], [0, 0]]
    pad_val = [[-1, -1], [-1, -1]]
    for n2 in range(len(joint_type.fixed.sides)):
        for oside in joint_type.fixed.sides[n2]:
            if oside.ax == joint_type.sax: continue
            axes = [0, 0, 0]
            axes[oside.ax] = 1
    ...
    for fixed_sides_1 in joint_type.fixed.sides:
        for fixed_sides_2 in joint_type.fixed.sides:
            for side1 in fixed_sides_1:
                if side1.ax == joint_type.sax: continue
                axes = [0, 0, 0]
                axes[side1.ax] = 1
                axes.pop(joint_type.sax)
                ax1 = axes.index(1)
                for side2 in fixed_sides_2:
                    if side2.ax == joint_type.sax: continue
                    axes = [0, 0, 0]
                    axes[side2.ax] = 1
                    axes.pop(joint_type.sax)
                    ax2 = axes.index(1)
    ...
    for lay_num in range(type.dim):
        ...
        for reg_num in range(type.dim * type.dim):
            ...
            for isl_num in range(10):
                ...
                outline, corner_artifacts = offset_verts(type, neighbor_vectors, neighbor_vectors_a, neighbor_vectors_b,
