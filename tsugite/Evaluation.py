...
def get_chessboard_vertics(mat: ArrayLike, ax: ArrayLike, noc: ArrayLike, n: ArrayLike) -> ArrayLike:
    chess = False
    dim = len(mat)
    verts = []
    for i in range(dim):
...
def get_region_outline(reg_inds: ArrayLike, lay_mat: ArrayLike, fixed_neighbors: ArrayLike, n: ArrayLike) \
    ...
    for i in range(lay_mat.shape[0]+1):
        for j in range(lay_mat.shape[1]+1):
            ...
            neighbors = np.array(neighbors)
            if np.any(neighbors.flatten()==0) and not np.all(neighbors.flatten()==0): # some but not all region neighbors
                dia1 = neighbors[0][1]==neighbors[1][0]
                dia2 = neighbors[0][0]==neighbors[1][1]
                if np.sum(neighbors.flatten()==0)==2 and  np.sum(neighbors.flatten()==1)==2 and dia1 and dia2: # diagonal detected
