...
class JointType:
    ...
    def init_gallery(self, start_index):
        ...
        location += os.sep + "search_results" + os.sep + "noc_" + str(self.noc) + os.sep + "dim_" + str(
            self.dim) + os.sep + "fs_"
    ...
    def open(self, filename="joint.tsu"):
        ...
        for i, line in enumerate(file.readlines()):
            ...
            if items[0] == "SAX":
                ...
                fs = FixedSides(self, side_str=items[1]).sides
            elif items[0] == "HFS":
                hfi = i
            elif i > hfi:
                hf = []
    ...
    def arrow_vertices(self) -> ArrayLike:
        vertices = []
        r = g = b = 0.0
        tx = ty = 0.0
    ...
    def pad_layer_mat_with_fixed_sides(self, mat, n):
        ...
        # if abs(self.angle-90)>1 and len(self.fixed.sides[n])==1 and self.fixed.sides[n][0].ax!=self.sax:
        #    print("get here")
        #    ax = self.fixed.sides[n][0].ax
        #    direction = self.fixed.sides[n][0].direction
        #    odir = 1-direction
        #    axes = [0,0,0]
        #    axes[ax] = 1
        #    axes.pop(self.sax)
    ...
    def milling_path_vertices(self, n):
        ...
        no_z = int(self.ratio * self.voxel_sizes[self.sax] / self.fab.depth)
        ...
        for lay_num in range(self.dim):
            ...
            for reg_num in range(self.dim * self.dim):
                ...
                edge_path = []
                if abs(self.ang) > 1: edge_path = self.edge_milling_path(lay_num, n)
                if len(edge_path) > 0:
        ...
        # Add end point
        end_verts, end_mverts = self.get_milling_end_points(n, milling_vertices[-1].pt[self.sax])
    ...
    def rough_milling_path(self, rough_pixs, lay_num, n):
        ...
        no_lanes = 2 + math.ceil(((self.real_tim_dims[axes[1]] / self.dim) - 2 * self.fab.diameter) / self.fab.diameter)
        ...
        for pix in rough_pixs:
            ...
            ind.insert(ax, (self.dim - 1) * (1 - direction) + (2 * direction - 1) * lay_num)  # 0 when n is 1, dim-1 when n is 0
            add = [0, 0, 0]
            add[ax] = 1 - direction
    ...
    def edge_milling_path(self, lay_num, n):
        ...
        if len(self.fixed.sides[n]) == 1 and self.fixed.sides[n][0].ax != self.sax:
            ...
            if not free:
                ...
                off_vec = (2 * direction - 1) * self.fab.vrad * off_vec
                pt0 = pt0 + off_vec
                pt1 = pt1 + off_vec
    ...
    def offset_verts(self, neighbor_vectors, neighbor_vectors_a, neighbor_vectors_b, verts, lay_num, n):
        ...
        for i, rv in enumerate(list(verts)):  # browse each vertex in the outline
            ...
            if rounded:
                ...
                le2 = math.sqrt(math.pow(2 * np.linalg.norm(off_vec_a + off_vec_b), 2) - math.pow(2 * self.fab.vrad,
                                                                                                  2)) - np.linalg.norm(
                    off_vec_a)
                ...
                if dist > self.fab.vdia and lay_num < self.dim - 1:
                    ...
                    while linalg.norm(pts3[2] - pts3[0]) > self.fab.vdia:
                        pts3[0] += vp
                        pts3[1] += -off_vec
                        pts3[2] += -vp
...
def is_additional_outer_corner(joint_type, rv, ind, ax, n):
    ...
    if rv.region_count == 1 and rv.block_count == 1:
        other_fixed_sides = joint_type.fixed.sides.copy()
        ...
        for sides in other_fixed_sides:
            for side in sides:
                ...
                if rv.ind[oax] == odir * joint_type.dim:
                    if rv.ind[not_oax] != 0 and rv.ind[not_oax] != joint_type.dim:
                        outer_corner = True
                        break
