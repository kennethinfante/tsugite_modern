...
class Geometries:
    def __init__(self, joint_type, main_mesh=True, hfs=[]):       # joint_type is JointType
        self.main_mesh = main_mesh
        self.joint_type = joint_type
        self.fab_directions = [0, 1]  # Initiate list of fabrication directions
        for i in range(1, self.joint_type.noc - 1): self.fab_directions.insert(1, 1)
        if len(hfs) == 0:
            self.height_fields = get_random_height_fields(self.joint_type.dim,
                                                          self.joint_type.noc)  # Initiate a random joint geometry
        else:
    ...
    def voxel_matrix_from_height_fields(self, first=False):
        vox_mat = mat_from_fields(self.height_fields, self.joint_type.sax)
    ...
    def create_indices(self, glo_off=0, milling_path=False):
        ...
        if not self.main_mesh:  # for suggestions and gallery - just show basic geometry - no feedback - global offset necessary
            for n in range(self.joint_type.noc):
                ax = self.joint_type.fixed.sides[n][0].ax
                nend, end, all, all_inds = self.joint_face_indices(all_inds,
                                                                   self.voxel_matrix, self.joint_type.fixed.sides[n], n,
                                                                   ax * self.joint_type.vn, global_offset=glo_off)
                lns, all_inds = self.joint_line_indices(all_inds, n, ax * self.joint_type.vn, global_offset=glo_off)
        ...
        # current geometry (main including feedback)
            ...
            for n in range(self.joint_type.noc):
                ...
                if not self.eval.bridged[n]:
                    ...
                    for m in range(2):
                        fne, fe, unbri, all_inds = self.joint_face_indices(all_inds,
                                                                           ...
                                                                           [self.joint_type.fixed.sides[n][m]], n,
                                                                           n * self.joint_type.vn)
                        unbris.append(unbri)
                else:
                    unbris = None
                ...
                # Lines
                lns, all_inds = self.joint_line_indices(all_inds, n, ax * self.joint_type.vn)
                ...
                # Opening lines
                open_indices, all_inds = self.open_line_indices(all_inds, n, ax * self.joint_type.vn)
                self.indices_open_lines.append(open_indices)

                # arrows
                larr, farr, all_inds = self.arrow_indices(all_inds, self.eval.slides[n], n, 3 * self.joint_type.vn)
                arrows = [larr, farr]

                if milling_path and len(self.joint_type.mverts[0]) > 0:
                    mill, all_inds = self.milling_path_indices(all_inds, int(len(self.joint_type.mverts[n]) / 8),
            ...
            if self.select.n is not None and self.select.new_fixed_sides_for_display is not None:
                self.outline_selected_component, all_inds = self.component_outline_indices(all_inds,
                                                                                           ...
                                                                                           self.select.n,
                                                                                           self.select.n * self.joint_type.vn)
        self.indices = all_inds

    def randomize_height_fields(self):
        self.height_fields = get_random_height_fields(self.joint_type.dim, self.joint_type.noc)
        self.voxel_matrix_from_height_fields()
        self.joint_type.combine_and_buffer_indices()

    def clear_height_fields(self):
        self.height_fields = []
        for n in range(self.joint_type.noc - 1):
            hf = np.zeros((self.joint_type.dim, self.joint_type.dim))
    ...
    def load_search_results(self, index=-1):
        ...
        location += os.sep + "search_results" + os.sep + "noc_" + str(self.joint_type.noc) + os.sep + "dim_" + str(
            self.joint_type.dim) + os.sep + "fs_"
        for i in range(len(self.joint_type.fixed.sides)):
            for fs in self.joint_type.fixed.sides[i]:
                location += str(fs[0]) + str(fs[1])
            if i != len(self.joint_type.fixed.sides) - 1: location += "_"
        ...
        self.fab_directions = []
        for i in range(self.joint_type.noc):
            if i == 0:
    ...
    def edit_height_fields(self, faces, h, n, direction):
        for ind in faces:
            ...
            if direction == 0:  # If editing top
                ...
                for i in range(n - direction + 1, self.joint_type.noc - 1):
        ...
        for i in range(d):
            for j in range(d):
                for k in range(d):
                    ...
                    for ax in range(3):
                        ...
                        if np.any(test_ind == self.joint_type.dim): continue
                        ...
                        if cnt == 1:
                            for x in range(2):
                                for y in range(2):
                                    add = [x, abs(y - x)]
                                    add.insert(ax, 0)
                                    index = get_index(ind, add, self.joint_type.dim)
            ...
            if len(fixed_sides) > 0:
                for side in fixed_sides:
                    a1, b1, c1, d1 = get_corner_indices(side.ax, side.direction, self.joint_type.dim)
                    step = 2
                    if len(self.joint_type.fixed.sides[n]) == 2: step = 1
    ...
    def joint_area_face_indices(self, all_indices, mat, area_faces, n):
        ...
        for i in range(d):
            for j in range(d):
                for k in range(d):
                    ind = [i, j, k]
                    for ax in range(3):
                        offset = ax * self.joint_type.vn
                        test_ind = np.array([i, j, k])
                        test_ind = np.delete(test_ind, ax)
                        if np.any(test_ind == self.joint_type.dim): continue
                        cnt, vals = face_neighbors(mat, ind, ax, n, self.joint_type.fixed.sides[n])
                        if cnt == 1:
                            for x in range(2):
                                for y in range(2):
                                    add = [x, abs(y - x)]
        ...
        d = self.joint_type.dim + 1
        start = d * d * d
        if len(self.joint_type.fixed.sides[n]) > 0:
            for side in self.joint_type.fixed.sides[n]:
                offset = side.ax * self.joint_type.vn
                a1, b1, c1, d1 = get_corner_indices(side.ax, side.direction, self.joint_type.dim)
                step = 2
                if len(self.joint_type.fixed.sides[n]) == 2: step = 1
                off = 24 * side.ax + 12 * side.direction + 4 * step
    ...
    def joint_line_indices(self, all_indices, n, offset, global_offset=0):
        fixed_sides = self.joint_type.fixed.sides[n]
        d = self.joint_type.dim + 1
        indices = []
        for i in range(d):
            for j in range(d):
                for k in range(d):
                    ind = [i, j, k]
                    for ax in range(3):
                        if ind[ax] == self.joint_type.dim: continue
                        ...
                        if cnt == 1 or cnt == 3 or (cnt == 2 and diagonal):
                            add = [0, 0, 0]
                            add[ax] = 1
                            start_i = get_index(ind, [0, 0, 0], self.joint_type.dim)
                            end_i = get_index(ind, add, self.joint_type.dim)
    ...
    def line_neighbors(self, ind, ax, n):
        ...
        for i in range(-1, 1):
            for j in range(-1, 1):
                ...
                if np.all(ind2 >= 0) and np.all(ind2 < self.joint_type.dim):
                    val = self.voxel_matrix[tuple(ind2)]
                else:
                    for n2 in range(self.joint_type.noc):
                        for side in self.joint_type.fixed.sides[n2]:
                            ind3 = np.delete(ind2, side.ax)
                            if np.all(ind3 >= 0) and np.all(ind3 < self.joint_type.dim):
                                if ind2[side.ax] < 0 and side.direction == 0:
                                    val = n2
                                elif ind2[side.ax] >= self.joint_type.dim and side.direction == 1:
                                    val = n2
                values.append(val)
    ...
    def chess_line_indices(self, all_indices, chess_verts, n, offset):
        ...
        for vert in chess_verts:
            add = [0, 0, 0]
            st = get_index(vert, add, self.joint_type.dim)
            add[self.joint_type.sax] = 1
            en = get_index(vert, add, self.joint_type.dim)
    ...
    def break_line_indices(self, all_indices, break_inds, n, offset):
        ...
        for ind3d in break_inds:
            add = [0, 0, 0]
            ind = get_index(ind3d, add, self.joint_type.dim)
            indices.append(ind)
        # Format
        indices = np.array(indices, dtype=np.uint32)
        indices = indices + offset
    ...
    def open_line_indices(self, all_indices, n, offset):
        ...
        if n == 0:
            directions = [0]
        elif n == self.joint_type.noc - 1:
            directions = [1]
        d = self.joint_type.dim + 1
        start = d * d * d

        for direction in directions:
            a1, b1, c1, d1 = get_corner_indices(self.joint_type.sax, direction, self.joint_type.dim)
            off = 24 * self.joint_type.sax + 12 * (1 - direction)
            a0, b0, c0, d0 = start + off, start + off + 1, start + off + 2, start + off + 3
            indices.extend([a0, a1, b0, b1, c0, c1, d0, d1])
    ...
    def joint_top_face_indices(self, all_indices, n, noc, offset):
        ...
        indices_tops = []
        indices = []
        sax = self.joint_type.sax
        for ax in range(3):
            for i in range(self.joint_type.dim):
                for j in range(self.joint_type.dim):
                    top_face_indices_cnt = 0
                    for k in range(self.joint_type.dim + 1):
                        if sdirs[0] == 0: k = self.joint_type.dim - k
                        ind = [i, j]
                        ind.insert(ax, k)
                        # count number of neigbors (0, 1, or 2)
                        cnt, vals = face_neighbors(self.voxel_matrix, ind, ax, n, self.joint_type.fixed.sides[n])
                        on_free_base = False
                        # add base if edge component
                        if ax == sax and ax != self.joint_type.fixed.sides[n][0].ax and len(sdirs) == 1:
                            base = sdirs[0] * self.joint_type.dim
        ...
        for face in select.faces:
            ...
            for i in range(2):
                ...
                for j in range(2):
                    ...
                    if np.all(nface >= 0) and np.all(nface < self.joint_type.dim):
                        unique = True
    ...
    def component_outline_indices(self, all_indices, fixed_sides, n, offset):
        ...
        if len(fixed_sides) == 2:
            ...
            a1, b1, c1, d1 = get_corner_indices(ax, 1 - direction, self.joint_type.dim)
