...
class Show:
    ...
    def update(self) -> None:
        ...
        if (self.view.open_joint and self.view.open_ratio < self.joint_type.noc - 1) or (
                not self.view.open_joint and self.view.open_ratio > 0):
            self.view.set_joint_opening_distance(self.joint_type.noc)
    ...
    def create_color_shaders(self) -> None:
        ...
        self.shader_col = OpenGL.GL.shaders.compileProgram(
    ...
    def create_texture_shaders(self) -> None:
        ...
        # Compiling the shaders
        self.shader_tex = OpenGL.GL.shaders.compileProgram(
            OpenGL.GL.shaders.compileShader(vertex_shader, GL_VERTEX_SHADER),
            OpenGL.GL.shaders.compileShader(fragment_shader, GL_FRAGMENT_SHADER))
    ...
    def draw_geometries(self, geos, clear_depth_buffer=True, translation_vec=np.array([0, 0, 0])):
        ...
        move_vec = [0, 0, 0]
        move_vec[self.joint_type.sax] = self.view.open_ratio * self.joint_type.component_size
        move_vec = np.array(move_vec)
        moves = []
        for n in range(self.joint_type.noc):
            tot_move_vec = (2 * n + 1 - self.joint_type.noc) / (self.joint_type.noc - 1) * move_vec
    ...
    def pick(self, xpos, ypos, height):
        ...
        if not self.view.gallery:
            ...
            col_step = 1.0 / (2 + 2 * self.joint_type.dim * self.joint_type.dim)
            for n in range(self.joint_type.noc):
                col = np.zeros(3, dtype=np.float64)
                col[n % 3] = 1.0
                if n > 2: col[(n + 1) % self.joint_type.dim] = 1.0
                ...
                if n == 0 or n == self.joint_type.noc - 1:
                    mos = 1
                else:
                    mos = 2
                ...
                for m in range(mos):
                    # Draw top faces
                    for i in range(self.joint_type.dim * self.joint_type.dim):
                        col -= col_step
                        glUniform3f(5, col[0], col[1], col[2])
                        top = ElementProperties(GL_QUADS, 4,
                ...
                if len(non_zeros) > 0:
                    if len(non_zeros[0] > 0):
                        ...
                        if len(non_zeros[0]) > 1:
                            pick_n = pick_n + self.joint_type.dim
                        ...
                        if i >= 0:
                            pick_x = (int(i / self.joint_type.dim)) % self.joint_type.dim
                            pick_y = i % self.joint_type.dim
                        pick_d = 0
                        if pick_n == self.joint_type.noc - 1:
                            pick_d = 1
                        elif int(i / self.joint_type.dim) >= self.joint_type.dim:
                            pick_d = 1
    ...
    def selected(self):
        ...
        if self.joint_type.mesh.select.state == 0:
            ...
            for face in self.joint_type.mesh.select.faces:
                if self.joint_type.mesh.select.n == 0 or self.joint_type.mesh.select.n == self.joint_type.noc - 1:
                    mos = 1
                else:
                    mos = 2
                index = int(self.joint_type.dim * face[0] + face[1])
                top = ElementProperties(GL_QUADS, 4, self.joint_type.mesh.indices_fpick_top[
        ...
        if self.joint_type.mesh.select.state == 2:
            ...
            for val in range(0, abs(self.joint_type.mesh.select.val) + 1):
                ...
                pulled_vec = [0, 0, 0]
                pulled_vec[self.joint_type.sax] = val * self.joint_type.voxel_sizes[self.joint_type.sax]
    ...
    def arrows(self):
        ...
        for n in range(self.joint_type.noc):
            ...
            if len(self.joint_type.fixed.sides[n]) == 2: d0 = d1
